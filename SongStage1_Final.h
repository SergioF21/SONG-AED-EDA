...
inline void ejecutarEtapa1_Mariel(MemoriaCompartida &shared_mem)
{
    ...
    for (int i = 0; i < DEGREE_LIMIT; ++i)
    {
        ...
        if (index_memoria >= memoria_global_grafo.size())
            break;
...
// Función principal encapsulada CON RETORNO DE CANDIDATOS
inline std::vector<int> runSongSimulation()
{
    ...
    // 4. PREPARAR RESULTADOS PARA RETORNAR
    std::vector<int> candidatos_encontrados;
    for (int i = 0; i < memoria.conteo_candidatos; i++)
    {
        candidatos_encontrados.push_back(memoria.lista_candidatos[i]);
    }

    // 5. MOSTRAR RESULTADOS
    std::cout << "\n=== RESULTADO FINAL STAGE 1 (Input para Stage 2) ===" << std::endl;
    std::cout << "Candidatos encontrados: [ ";
    for (int i = 0; i < candidatos_encontrados.size(); i++)
    {
        std::cout << candidatos_encontrados[i] << " ";
    }
    std::cout << "]" << std::endl;

    // 6. RETORNAR LOS CANDIDATOS PARA LA SIGUIENTE ETAPA
