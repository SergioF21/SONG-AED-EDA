...
////////////////////////////////////////////////////////////////////////////////
// CONFIGURABLE CONSTANTS (recompile if you change)
////////////////////////////////////////////////////////////////////////////////
#define DEGREE_DEFAULT 16    // default fixed-degree; override at runtime if needed
#define DIM_DEFAULT    128   // default dimension used to size shared arrays (safe upper bound)
#define TOPK_DEFAULT   10
#define Q_CAP_DEFAULT  (4 * TOPK_DEFAULT)     // bounded queue capacity
#define VISITED_SIZE_DEFAULT 64               // should be power of two (>= 4*TOPK recommended)

#define THREADS_PER_BLOCK 128
#define WARP_SIZE 32
#define CUDA_CHECK(call) \
  do { cudaError_t rc = (call); if (rc != cudaSuccess) { \
    fprintf(stderr, "CUDA ERROR %s:%d: %s\n", __FILE__, __LINE__, cudaGetErrorString(rc)); exit(1); } } while(0)
...
__global__ void song_kernel(
    ...
    // thread 0 initializes control structures
    if (tid == 0) {
        *s_candidate_count = 0;
        *s_q_size = 0;
        *s_topk_size = 0;
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
        // ---------------- Stage 1: Candidate Locating (thread 0) ----------------
        if (tid == 0) {
            int qsz = *s_q_size;
            if (qsz == 0) {
                *s_candidate_count = 0;
        ...
        if (candidate_count <= 0) {
            // termination condition: empty queue => finish
            break;
        }

        // ---------------- Stage 2: Bulk Distance Computation (parallel) ----------------
        // assign one candidate per warp
        for (int c = warpId; c < candidate_count; c += numWarps) {
            ...
            if (lane == 0) {
                s_cand_dist[c] = sum; // squared L2
            }
        }
        __syncthreads();

        // ---------------- Stage 3: Data Structure Maintenance (thread 0) ----------------
        if (tid == 0) {
            int now = *s_now_idx;
...
////////////////////////////////////////////////////////////////////////////////
// HOST: helpers to load files, launch kernel and print results
////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char** argv) {
    ...
    int VISITED_SIZE = VISITED_SIZE_DEFAULT;
    int Q_CAP = Q_CAP_DEFAULT;
    ...
    shared_size += sizeof(float) * dim;                // s_query
    shared_size += sizeof(int32_t) * graph_degree;     // s_candidates
    shared_size += sizeof(float) * graph_degree;       // s_cand_dist
    shared_size += sizeof(int32_t) * Q_CAP;            // s_q_ids
    shared_size += sizeof(float) * Q_CAP;              // s_q_dists
    shared_size += sizeof(int) * 1;                    // s_q_size
    shared_size += sizeof(int32_t) * TOPK;             // s_topk_ids
    shared_size += sizeof(float) * TOPK;               // s_topk_dists
    shared_size += sizeof(int) * 1;                    // s_topk_size
    shared_size += sizeof(int32_t) * VISITED_SIZE;     // s_visited
    shared_size += sizeof(int) * 1;                    // s_candidate_count
    shared_size += sizeof(int) * 1;                    // s_now_idx
    ...
    song_kernel<<<grid, block, shared_size>>>(
